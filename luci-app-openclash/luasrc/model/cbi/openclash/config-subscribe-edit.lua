...
if token then
	get_sub = string.format("curl -sL -H 'Content-Type: application/json' --connect-timeout 2 -d '{\"access_token\":\"%s\"}' -X POST https://dler.cloud/a$(TOPDIR)/feeds/packages/managed/clash -o %s", token, sub_path)
