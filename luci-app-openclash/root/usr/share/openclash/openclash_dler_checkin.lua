...
local function dler_checkin()
	...
	if token and email and passwd and enable == "1" then
		checkin = string.format("curl -sL -H 'Content-Type: application/json' -d '{\"email\":\"%s\", \"passwd\":\"%s\", \"multiple\":\"%s\"}' -X POST https://dler.cloud/a$(TOPDIR)/feeds/packages/checkin -o %s", email, passwd, multiple, path)
